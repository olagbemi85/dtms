...
def nin_validate_length(value):
    ...
    if length > 6:
        raise ValidationError(
            gettext_lazy('%(value)s is above 6 digits')
        )
    if length < 6:
        raise ValidationError(
            gettext_lazy('%(value)s is below 6 digits')
        )
...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    staff_id_number = models.IntegerField (validators=[nin_validate_length], unique=True)
    ...
    REQUIRED_FIELDS = ['email','first_name', 'last_name','staff_id_number']
