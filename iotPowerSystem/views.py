...
#from authentications.decorators import login_required
...
import datetime
from authentications.models import Notification
...
def index(request):
    return render(request, 'iotPowerSystem/frontpage.html')
...
def index(request):
    if request.user.is_authenticated:
        ...
        return redirect('authentications:userlogin')                          


@login_required
def profileRegister(request):
    profile_form = UserSPSForm(request.POST, instance=request.user.user)
    context = {'profile_form':profile_form}
    if profile_form.is_valid():
        profile_form.save()
        messages.success(request, 'Your profile is updated successfully')
